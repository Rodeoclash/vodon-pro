...
contextBridge.exposeInMainWorld("app", {
  ...
  onVideoThumbnailGenerationProgress: (
    callback: (event: any, progress: VideoThumbnailGenerationProgress) => void
...
contextBridge.exposeInMainWorld("video", {
  ...
  generateThumbnails: (filePath: string) =>
    ipcRenderer.invoke("video:generateThumbnails", filePath),
});
